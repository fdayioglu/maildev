...
web.start = function (port, host, mailserver, user, password, basePathname, secure) {
  ...
  // Parse URL-encoded form bodies for login form
  ...
  if (user && password) {
    ...
    app.get(loginPath, function (req, res) {
      ...
      if (auth.isValidToken(cookies.maildev_auth)) {
        return res.redirect(basePathname || '/')
      }
      res.sendFile(path.join(__dirname, '../app/login.html'))
    })

    // Handle login form submission (no auth required)
    app.post(loginPath, function (req, res) {
      if (req.body.username === user && req.body.password === password) {
        const token = auth.createToken()
        res.cookie('maildev_auth', token, {
          httpOnly: true,
          sameSite: 'strict',
          secure: !!secure.https
        })
      ...
      res.redirect(loginPath + '?error=1')
    })

    // Handle logout
    app.get(logoutPath, function (req, res) {
      const cookies = auth.parseCookies(req)
      if (cookies.maildev_auth) {
        auth.revokeToken(cookies.maildev_auth)
      }
      res.cookie('maildev_auth', '', { httpOnly: true, sameSite: 'strict', maxAge: 0 })
  ...
  routes(app, mailserver, basePathname, !!(user && password))
