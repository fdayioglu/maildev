...
/**
 * Remove emails older than 48 hours from memory and disk
 */
...
function purgeOldEmails () {
  ...
  for (let i = store.length - 1; i >= 0; i--) {
    const email = store[i]
    const emailTime = new Date(email.time).getTime()
    if (emailTime < cutoff) {
      fs.unlink(path.join(mailServer.mailDir, email.id + '.eml'), function (err) {
        if (err && err.code !== 'ENOENT') logger.error(err)
      })
      if (email.attachments) {
        fs.rm(path.join(mailServer.mailDir, email.id), { recursive: true }, function (err) {
          if (err && err.code !== 'ENOENT') logger.error(err)
        })
      }
      store.splice(i, 1)
      eventEmitter.emit('delete', { id: email.id, index: i })
      logger.log('Purged expired email: %s', email.subject)
    }
  }
}
...
mailServer.listen = function (callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    mailServer.smtp.listen(mailServer.port, mailServer.host, function (err) {
      ...
      // Start periodic purge of emails older than 48 hours (check every hour)
      purgeOldEmails()
      purgeTimer = setInterval(purgeOldEmails, 60 * 60 * 1000)
    })
  })
}
